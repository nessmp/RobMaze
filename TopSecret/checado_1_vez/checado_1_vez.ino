...
void MoverseAProx(int iHere, int iHelper)
{
...
void Go(int SaveGo)
{
  ...
  while(YouAreHere != iGetHere)
  {
    ...
    MoverseAProx(iProx,iHelper);
    YouAreHere = iPos[iProx];
    CopyCounter--;
  }
}

//Moverse Despues de haberse movido al ipass a lado de la ubicacion deseada
void MoverseUltimo(int HelperToMove, int SaveGo)
{
  int GoHere = HelperToMove;

    if(iPos[SaveGo] + 100 == GoHere) 
    {
      Derecha30();
      iCoord += 100;
    }
    else if(iPos[SaveGo] + 1 == GoHere) 
    {
      Adelante30();
      iCoord += 1;
    }
    else if(iPos[SaveGo] - 100 == GoHere) 
    {
      Izquierda30();
      iCoord -= 100;
    }
...
void Laberinto()
{
    ...
    if(Next == true)
    {
        SaveGo = SearchWhereToGo(HelperToMove);
        Go(SaveGo);
        MoverseUltimo(HelperToMove, SaveGo);
    }
}
